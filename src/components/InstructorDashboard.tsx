...
export const InstructorDashboard: React.FC<InstructorDashboardProps> = ({ instructorEmail, instructorName }) => {
  ...
  const fetchStudents = async (signal?: AbortSignal) => {
    ...
    setFetchError('');
    try {
      const { data: users, response: res } = await api.get<any[]>('/institutions/users', { signal });
      if (res.ok && Array.isArray(users)) {
        const mapped = users.map((u: any) => {
          const progresses = u.courseProgresses || [];
          const totalCertificates = progresses.filter((p: any) => p.status === 'APPROVED' || p.status === 'CERTIFIED').length;
          return {
            id: u.id,
            name: u.name,
            email: u.email,
            ...
            progresses: progresses.map((p: any) => ({
              courseId: p.courseId,
              status: p.status,
              passedLessonsCount: (p.status === 'APPROVED' || p.status === 'CERTIFIED') ? 100 : 0
            }))
          };
        });
        setStudents(mapped);
        ...
        setFetchError(`El servidor respondió con error (${res.status}).`);
      }
    } catch (e: any) {
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    setAddFeedback('');
    try {
      const { data, response: res } = await api.post('/institutions/add-user', { name: newName, email: newEmail });
      if (res.ok) {
        setAddFeedback(`¡${newName} matriculado exitosamente!`);
        await fetchStudents();
        setTimeout(() => { setIsAddOpen(false); setNewName(''); setNewEmail(''); setAddFeedback(''); }, 1500);
      } else {
        setAddFeedback(`Error: ${data?.error || 'No se pudo agregar al alumno'}`);
      }
    } catch (err: any) {
  ...
  const handleApprove = async (e: React.FormEvent) => {
    ...
    setApprovalFeedback('');
    try {
      const { data, response: res } = await api.post('/institutions/emit-recognition', {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-[#181818] border border-gray-200 dark:border-[#333333] rounded-xl p-5 gcp-card-shadow flex items-center space-x-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Lecciones</p>
        ...
        <div className="bg-white dark:bg-[#181818] border border-gray-200 dark:border-[#333333] rounded-xl p-5 gcp-card-shadow flex items-center space-x-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Certificados</p>
        ...
        <div className="bg-white dark:bg-[#181818] border border-gray-200 dark:border-[#333333] rounded-xl p-5 gcp-card-shadow flex items-center space-x-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Suscripción</p>
            <span className="inline-block mt-0.5 px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
              Plan Activo
            </span>
          </div>
        </div>
